...
function blobCredentialsConfigured(env: RuntimeEnvironment) {
  return (
    configured(env, 'NAGARIK_BLOB_READ_WRITE_TOKEN') ||
...
export function deploymentRelease(env: RuntimeEnvironment) {
  return {
    ...
    commitSha:
      cleanCommit(env.VERCEL_GIT_COMMIT_SHA) ??
...
export function runtimeReadiness(input: RuntimeReadinessInput) {
  ...
  const mediaStorage =
    input.mediaStorageMode !== null &&
    (!production || input.mediaStorageMode === 'blob') &&
    (input.mediaStorageMode !== 'blob' || blobCredentials);
  const sessionSecurity =
    !production ||
    ((configured(input.env, 'NAGARIK_COOKIE_SECRET') ||
      configured(input.env, 'NAGARIK_SESSION_DERIVATION_SECRET')) &&
      configuredBytes(input.env, 'NAGARIK_SESSION_DERIVATION_SECRET', 32) &&
      (configured(input.env, 'NAGARIK_UPLOAD_RECEIPT_SECRET') ||
        configured(input.env, 'NAGARIK_COOKIE_SECRET')));
  const abuseProtection =
    !production ||
    (configured(input.env, 'NAGARIK_RATE_LIMIT_SALT') &&
      (configured(input.env, 'NAGARIK_RATE_LIMIT_PEPPER') ||
        configured(input.env, 'NAGARIK_COOKIE_SECRET')));
  ...
  const operational =
    publicRead && (input.publicPreviewReadOnly || (reporting && stewardship && maintenance));
