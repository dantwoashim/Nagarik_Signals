...
|---|---|---|
| defect | fail | confirmed baseline P0-P2 findings |
| correctness/consistency | fail | chain-first JSON prototype |
| privacy/authorization | fail | public-before-moderation, orphan media, shared secret, public steward queue |
| smart contract | fail | v2 absent; v1 only |
| API/workflow | fail | production v2 endpoints/services absent |
| UX/accessibility | missing evidence | prototype E2E is not production matrix |
| operations/recovery/security | fail/missing | audits, restore, alerts, full CI absent |
| external human | blocked | no approved evidence supplied |
